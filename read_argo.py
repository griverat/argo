...
def get_prof(data_file, lat_range, lon_range):
    ...
    mask = (
        (data_file.LATITUDE > lat_range[0])
        & (data_file.LATITUDE < lat_range[1])
        & (data_file.LONGITUDE > lon_range[0])
        & (data_file.LONGITUDE < lon_range[1])
    )
...
def main(ndates, name, lat_range, lon_range=[-180, 180]):
    ...
    platf = (
        ...
        .drop_duplicates(subset=["PLATFORM_NUMBER"])
    )
